...
export class GameRoom extends Room<GameState> {
  ...
  onJoin(client: Client, options: { name: string }, auth?: any) {
    ...
    const players = [...this.state.players].sort(
      (a, b) => a[1].order - b[1].order
    );
    const order =
      players.length > 0 ? players[players.length - 1][1].order + 1 : 0;
    this.state.players.set(
      client.sessionId,
      new Player({ id: client.sessionId, name, order })
    );
